...
public class BoardHandler extends AbstractHandler {
  ...
  public BoardHandler(String filename) { // fileName을 받을 때 boardDao로 가서 생성자를 넘겨 줌
    // 수퍼 클래스의 생성자를 호출할 때 메뉴 목록을 전달한다.
    super(new String[] {"목록", "상세보기", "등록", "삭제", "변경"});

    boardDao = new BoardDao(filename); // 지금처럼 생성자의 파라미터 값을 가지고 인스턴스를 생성할 경우 여기 이렇게 해야함
  ...
  @Override
  public void service(int menuNo) { // 예외를 던질 수 없는 이유는 수퍼클래스에 있는 추상 메소드에 예외를 던질 수 있다고 선언을 안해서 그럼
    try {
      switch (menuNo) {
      ...
      throw new RuntimeException(e); // 그래서 몰래 던질 수 있는 RuntimeException에 담아서 던진다.
    }
  }
